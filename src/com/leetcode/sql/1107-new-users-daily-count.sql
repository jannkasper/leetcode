SELECT query.activity_date AS 'login_date', COUNT(*) AS 'user_count'
FROM (SELECT user_id, MIN(activity_date) as 'activity_date'
        FROM Traffic
        WHERE activity = 'login'
        GROUP BY user_id) as query
WHERE DATEDIFF(2019-06-30, query.activity_date) < 90
GROUP BY query.activity_date;
